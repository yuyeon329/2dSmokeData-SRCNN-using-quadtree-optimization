...
tf.app.flags.DEFINE_string('losses', 'mse', 'mse,perceptual,texture,adv')
...
def main(argv=None):
    ...
    # print(resized_low_images.shape, vis_gen_images.shape, vis_high_images.shape)
    ...
    if 'adv' in loss_list:
        ...
        gen_optimizer = utils.configure_optimizer(learning_rate) #tf.train.AdamOptimizer(learning_rate)
        gen_gradients = gen_optimizer.compute_gradients(generator_loss, var_list=generator_vars)
        gen_grad_updates = gen_optimizer.apply_gradients(gen_gradients)#, global_step=global_step)
    ...
    ######################### Train process
    #start
    #FLAGS -> 명령어로 직접 입력 image path ,batch_size hr_size // num_workers 는 queue에서 사용하는거라 앞에 세개만 보기!
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        if FLAGS.model_restore:          # w,b 불러오기(가장 최적화된 weight, bias 값을 가져오는거야!)
            ckpt = tf.train.latest_checkpoint(FLAGS.save_path)
            saver.restore(sess, ckpt)
        else:
            sess.run(tf.global_variables_initializer())       #변수설정 초기화 (항상 있는거야)
            if FLAGS.vgg_path is not None:
